<script setup>
import { computed, reactive, ref, onBeforeMount, toRaw, watch } from "vue";
  ...
  form.value.validate(async (valid, fields) => {
    if (valid) {
      if (state.mode) {
        ...
        let res = await axios.get("/ache/calendar/get-comments", { params: { pid: aSchedule.value.id } })
...
<template>
  ...
  <el-dialog v-model="state.showDialog" custom-class="my-dialog general">
    ...
    <template #footer>
      <el-button @click="operateSchedule()">确定</el-button>
    </template>
  </el-dialog>
  ...
  <el-dialog v-model="state.showDialog2" custom-class="my-dialog general">
    ...
    <el-form :model="comment" ref="form" :rules="rules2" :label-width="52">
